...
#include <sstream>
#endif
#ifdef XIAO_SPDLOG_SUPPORT
#include <spdlog/spdlog.h>
#include <map>
#include <mutex>
#endif // XIAO_SPDLOG_SUPPORT
...
namespace std
{
	inline xiao::Logger::LogLevel clamp(xiao::Logger::LogLevel v,
		xiao::Logger::LogLevel min,
		xiao::Logger::LogLevel max)
	{
...
#ifndef _MSC_VER
			strncpy(lastTimeString_, date_.toFormattedStringLocal(false).c_str(),
				sizeof(lastTimeString_) - 1);
#else
			strncpy_s<sizeof lastTimeString_>(
				lastTimeString_, date_.toFormattedStringLocal(false).c_str(),
				sizeof(lastTimeString_) - 1);
#endif  // !_MSC_VER
		}
		else {
...
static const char* logLevelStr[Logger::LogLevel::xNumberofLogLevels] = {
	" TRACE ",
	" DEBUG ",
	" INFO  ",
	" WARN  ",
...
Logger::Logger(SourceFile file, int line) 
	: sourceFile_(file), fileLine_(line), level_(xInfo)
{
	formatTime();
...
Logger::Logger(SourceFile file, int line, LogLevel level)
	:sourceFile_(file), fileLine_(line), level_(std::clamp(level, xTrace, xFatal))
{
	formatTime();
	logStream_ << T(logLevelStr[level_], 7);
#ifdef XIAO_SPDLOG_SUPPORT
...
Logger::Logger(SourceFile file, int line, LogLevel level, const char* func)
	...
	level_(std::clamp(level, xTrace, xFatal))
#ifdef XIAO_SPDLOG_SUPPORT
	,
	func_(func)
#endif
...
Logger::Logger(SourceFile file, int line, bool)
	: sourceFile_(file), fileLine_(line), level_(xFatal)
{
	formatTime();
	logStream_ << T(logLevelStr[level_], 7);
#ifdef XIAO_SPDLOG_SUPPORT
...
Logger::Logger(LogLevel level) : level_(std::clamp(level, xTrace, xFatal))
{
	formatTime();
	logStream_ << T(logLevelStr[level_], 7);
#ifdef XIAO_SPDLOG_SUPPORT
...
Logger::Logger(bool) : level_(xFatal)
{
	formatTime();
	logStream_ << T(logLevelStr[level_], 7);
#ifdef XIAO_SPDLOG_SUPPORT
